          ...
          <div className="flex bg-surface/60 rounded-xl p-1 border border-line">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${
                viewMode === 'browse' ? 'bg-primary text-white' : 'text-muted hover:text-white'
              }`}
            >
            ...
            <button
              onClick={startReview}
              className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${
                viewMode === 'review' ? 'bg-primary text-white' : 'text-muted hover:text-white'
              }`}
            >
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 content-start">
            {items.length === 0 ? (
              ...
              filteredItems.map((card) => (
                <div key={card.id} className="bg-dark/50 border border-line-strong rounded-xl p-4 hover:border-line-strong transition-colors group relative">
                  ...
                  <p className="text-muted text-xs mt-2 leading-snug border-l-2 border-line-strong pl-2">{card.reason}</p>
                  <div className="mt-3 flex justify-between text-[10px] text-faint">
                    <span className="px-1.5 py-0.5 rounded bg-surface-2 border border-line-strong">Box {card.box}</span>
