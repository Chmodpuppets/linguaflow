          ...
          <div className="flex bg-gray-900/60 rounded-xl p-1 border border-gray-800">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${
                viewMode === 'browse' ? 'bg-primary text-white' : 'text-gray-400 hover:text-white'
              }`}
            >
            ...
            <button
              onClick={startReview}
              className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${
                viewMode === 'review' ? 'bg-primary text-white' : 'text-gray-400 hover:text-white'
              }`}
            >
        ...
        <div className="max-w-2xl mx-auto w-full">
          {queue.length > 0 ? (
            <div className="bg-card border border-gray-700 rounded-2xl p-8 text-center">
              ...
              <div className="text-sm text-gray-500 mb-1">你的错误写法</div>
            ...
            <div className="bg-card border border-gray-700 rounded-2xl p-10 text-center">
              ...
              <p className="text-gray-400 text-sm mb-6">你刚刚复习了 {reviewDone} 个错题。间隔复习让弱项不再弱。</p>
              <button
                onClick={() => setViewMode('browse')}
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 content-start">
            {items.length === 0 ? (
              ...
              filteredItems.map((card) => (
                <div key={card.id} className="bg-dark/50 border border-gray-700 rounded-xl p-4 hover:border-gray-500 transition-colors group relative">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <button
                      ...
                      className="text-gray-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity p-1 ml-2 flex-shrink-0"
                      title="删除错题"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
                  <p className="text-gray-400 text-xs mt-2 leading-snug border-l-2 border-gray-600 pl-2">{card.reason}</p>
                  <div className="mt-3 flex justify-between text-[10px] text-gray-600">
                    <span className="px-1.5 py-0.5 rounded bg-gray-800 border border-gray-700">Box {card.box}</span>
