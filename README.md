...
LinguaFlow 围绕一个理念设计：**你学得最好的语言，是你用得最多的语言。** 所以应用里没有传统课本式的单选填空，而是把练习做成了 **17 个「上手就能玩」的模块**（覆盖输入、输出、复习、社交、复盘完整闭环）：
...
> 应用左侧导航共 **17 个模块**，覆盖「输入 → 输出 → 复习 → 社交 → 复盘」完整学习闭环。下面按导航顺序逐一说明。
...
### 11. 写作趋势（Writing Trends）
- 把字数 / 评分 / 错误类型做成可视化图表。
- 帮你定位薄弱项，指导下一阶段练习方向。

### 12. 文字特训（Script Trainer · 手写）
